...
void Sim800L::Start_HTTP(String url) {
    ...
    error = checkError("AT+HTTPPARA=CONTENT,application/x-www-form-urlencoded", 3000, error);
...
void Sim800L::Send_CONFIG_HTTP(uint16_t DataSize, uint16_t waitingTime, String Data) {
    ...
    int error = 0;
    error = checkError("AT+HTTPDATA=" + String(DataSize) + "," + String(waitingTime), 4000, error);
...
String Sim800L::Send_HTTP(uint8_t method) {
    ...
    checkError("AT+HTTPACTION="+String(method), 6000);
    SerialGSM.println("AT+HTTPREAD");

    return _readSerial(6000);
}
